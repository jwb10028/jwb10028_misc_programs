...
public:
    ...
    template <class T1>
    friend void setColor(RBTNode<T1>* point, int inColor);

    int height() const;
};

template <class T>
void setColor(RBTNode<T>* point, int inColor){
    point->color = inColor;
}
...
public:
    ...
    void insertDriver(RBTNode<T>* point);
...
template <class T>
void RBT<T>::insertDriver(RBTNode<T>* point){
    //define insertDriver here
